...
public class MessageServiceImpl implements MessageService{
    ...
    @Override
    public ResultInfo selectSystemMessages(int userId, int limit, int offset){    //limit是从0开始的
        ...
        listMessage = systemMessageDao.selectSystemMessageByToId(userId, limit, offset);
        ...
        if(listMessage!=null) {
            ...
            return ResultInfo.build(999,"读取动态失败");
    }
    @Override
    public List systemMessagesAmount(int userId ,int limit){
        int amount = systemMessageDao.systemMessageAmount(userId);
        amount = amount/12+1;        //一共amount页
